...
public class World {
	...
	private void generateLevel() {
			...
			for (int i = 0; i < numRivers; i++) {
				...
				if (checkOverlapPen(river) || checkOverlapRiver(river))
					i--;
				else
					rivers.add(river);
			}
			...
			for (int i = 0; i < numTrees; i++) {
				tree = new Tree(rand(0,
						(int) (WORLD_WIDTH - Tree.TREE_WIDTH / 2)), rand(0,
						(int) (WORLD_HEIGHT - Tree.TREE_HEIGHT / 2)));
				if (checkOverlapPen(tree) || checkOverlapRiver(tree))
					i--;
				else
					trees.add(tree);
			}
			...
			for (int i = 0; i < numSheeps; i++) {
				sheep = new Sheep(rand(50, (int) WORLD_WIDTH - 70), rand(50,
						180));
				sheep.rotation = rand(0, 360);
				if (checkOverlapObject(sheep))
					i--;
				else
					sheeps.add(sheep);
			}
		...
		for (Sheep sheep : sheeps) {
			if (sheep.state != Sheep.SHEEP_STATE_CATCHED) {
				if (sheep.bounds.overlaps(pen.bounds)
					...
					sheep.rotation += 180;
					sheep.position.add(-sheep.direction.x * 5,
							-sheep.direction.y * 5);
				} else if (pen.hasScored(sheep.bounds)) {
					sheep.state = Sheep.SHEEP_STATE_CATCHED;
					sheepsCollected++;
				}
			} else if (sheep.state == Sheep.SHEEP_STATE_CATCHED
					&& !pen.hasScored(sheep.bounds)) {
	...
	private void checkCollisionRiver() {
		for (Sheep sheep : sheeps) {
			for (River river : rivers) {
				if (sheep.bounds.overlaps(river.bounds)
						&& !river.canPass(sheep)) {
					sheep.rotation += 180;
					sheep.position.add(-sheep.direction.x * 5,
							-sheep.direction.y * 5);
