package me.teamsheepy.sheepherder;

import me.teamsheepy.sheepherder.screens.GameScreen;
import me.teamsheepy.sheepherder.utils.GoogleServices;
import me.teamsheepy.sheepherder.utils.TimeFormatter;

import com.badlogic.gdx.Game;
import com.badlogic.gdx.graphics.g2d.SpriteBatch;

public class SheepHerder extends Game {
	// used by all screens
	public SpriteBatch batcher;
	
	public static TimeFormatter timeFormatter = null;
	public static GoogleServices googleServices = null;

	public SheepHerder(GoogleServices googleServices, TimeFormatter timeFormatter) {
		SheepHerder.timeFormatter = timeFormatter;
		SheepHerder.googleServices = googleServices;
	}
	   
	@Override
	public void create () {
		batcher = new SpriteBatch();
		SavedData.load();
		Assets.load();
		setScreen(new GameScreen(this));
	}
	
	@Override
	public void render() {
		super.render();
	}
}
