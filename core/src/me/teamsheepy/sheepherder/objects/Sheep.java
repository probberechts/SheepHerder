...
public class Sheep extends DynamicGameObject {
	...
	public Sheep(float x, float y) {
		...
		collisionAreas.add(new Rectangle(this.bounds.x + 20,
				this.bounds.y + 20, this.bounds.width - 40,
	...
	public void updateSheepSpeed(float deltaTime, SheepWorld world) {
		...
		// Calculate the amount of X and Y movement expected by the sheep this
		// frame
		...
		Rectangle movementBounds = new Rectangle(Math.min(bounds.x, bounds.x
				+ nextMoveX), Math.min(bounds.y, bounds.y + nextMoveY),
				Math.max(bounds.width, bounds.width + nextMoveX), Math.max(
						bounds.height, bounds.height + nextMoveY));

		// Check each object to see whether it intersects with the sheep's
		// bounding box
		for (GameObject object : world.getWorldObjects()) {
			for (Rectangle collisionArea : object.collisionAreas) {
				if (object instanceof Sheep) {
					...
					Rectangle smaller = new Rectangle(collisionArea.x
							+ shrinkAmount, collisionArea.y + shrinkAmount,
							collisionArea.width - shrinkAmount,
							collisionArea.height - shrinkAmount);
	...
	public void updateStaticCollisions(float deltaTime, SheepWorld world) {
		...
		// Check each static object to see whether it intersects with the
		// sheep's bounding box
		...
		// Iterate over each object whose bounding box intersects with the
		// player's bounding box
		// until a collision is found
		safeMoveX = nextMoveX;
		safeMoveY = nextMoveY;
		float safeVecLen = (float) Math.sqrt(safeMoveX * safeMoveX + safeMoveY
				* safeMoveY);
		...
		for (Rectangle area : collisions) {
			...
			//
			// We will traverse along the movement vector of the sheep from its
			// current
			// position until the final position for the frame to check if any
			// object lies
			// in the way. If so, the vector is adjusted to end at the object's
			// intersection
			// with the sheep's movement vector. This solves the so-called
			// 'bullet through
			// paper' problem.
			// ================================================================================
			Rectangle intersection = intersect(movementBounds, area);
			float newSafeMoveX = Math.max(Math.max(intersection.x
					- (movementBounds.x + movementBounds.width),
					movementBounds.x - (intersection.x + intersection.width)),
					0);
			float newSafeMoveY = Math.max(Math.max(intersection.y
					+ intersection.height
					- (movementBounds.y + movementBounds.height),
					movementBounds.y - intersection.y), 0);
			float newSafeVecLen = (float) Math.sqrt(newSafeMoveX * newSafeMoveX
					+ newSafeMoveY * newSafeMoveY);
		...
		// If the sheep is pushed to a collision, let it move along the
		// colliding rectangle
		// TODO: Dit stuk is niet optimaal en werkt bijvoorbeeld enkel als er
		// met slechts één object een collison is.
		// Maar na enkele uren erop te zitten zoeken heb ik niets beter kunnen
		// vinden. Iemand een betere oplossing?
		// Bovendien bewegen de schaapjes zijdelings, maar als je hun richting
		// gaat omzetten in graden en je ze draait,
		// zorgen afrondingsfouten ervoor dat de schaapjes beginnen trillen.
		if (collisions.size == 1 && firstCollisonObject != null
				&& safeMoveX == 0 && safeMoveY == 0) {
			Vector2 nextMove = new Vector2(nextMoveX, nextMoveY);
			if (getCollisionSide(this.bounds, firstCollisonObject) == 0
					|| getCollisionSide(this.bounds, firstCollisonObject) == 2)
				safeMoveX = (float) (nextMove.len() * Math.cos(Math
						.toRadians(nextMove.angle())));
			if (getCollisionSide(this.bounds, firstCollisonObject) == 1
					|| getCollisionSide(this.bounds, firstCollisonObject) == 3)
				safeMoveY = (float) (nextMove.len() * Math.sin(Math
						.toRadians(nextMove.angle())));
		}
	...
	public void updateOld(float deltaTime, SheepWorld world) {
		...
		// and its expected movement this frame
		Rectangle movementBounds = new Rectangle(Math.min(bounds.x, bounds.x
				+ safeMoveX), Math.min(bounds.y, bounds.y + safeMoveY),
				Math.max(bounds.width, bounds.width + safeMoveX), Math.max(
						bounds.height, bounds.height + safeMoveY));
				...
				if (!object.equals(this)) {
					if (object instanceof Sheep) {
						...
						Vector2 predictedPos = new Vector2(
								((Sheep) object).body.getPosition().x,
								((Sheep) object).body.getPosition().y);
						predictedPos.add(((Sheep) object).safeMoveX,
								((Sheep) object).safeMoveY);
						predictedSprite.setPosition(predictedPos.x,
								predictedPos.y);

						Rectangle predictedBounds = predictedSprite
								.getBoundingRectangle();
						Rectangle collArea = new Rectangle(
								predictedBounds.x + 20, predictedBounds.y + 20,
								predictedBounds.width - 40,
								predictedBounds.height - 40);

						int shrinkAmount = 25;
						// make collisionArea smaller
						Rectangle smaller = new Rectangle(collArea.x
								+ shrinkAmount, collArea.y + shrinkAmount,
								collArea.width - shrinkAmount, collArea.height
										- shrinkAmount);
						if (smaller.overlaps(movementBounds)) {
							// sheepcollision
	...
	public void update(float deltaTime) {
		...
		if (timeToIdle > 0)
			...
			Vector2 force = new Vector2(
					(float) Math.cos(Math.toRadians(rot)) * 30000,
					(float) Math.sin(Math.toRadians(rot)) * 30000);
			body.applyLinearImpulse(force.x, force.y, body.getPosition().x,
	...
	private Rectangle intersect(Rectangle rectangle1, Rectangle rectangle2) {
		if (rectangle1.overlaps(rectangle2)) {
			...
			intersection.width = Math.min(rectangle1.x + rectangle1.width,
					rectangle2.x + rectangle2.width) - intersection.x;
			intersection.y = Math.max(rectangle1.y, rectangle2.y);
			intersection.height = Math.min(rectangle1.y + rectangle1.height,
					rectangle2.y + rectangle2.height) - intersection.y;
			return intersection;
		}
		return null;
	}
